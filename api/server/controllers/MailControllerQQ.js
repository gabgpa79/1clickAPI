...
class MailController {
  ...
  static sendPanico(user) {    
    return new Promise((resolve, reject) => {
      Promise.all([EmpresaService.getEmpresa(1)]).then(([empresa]) => {
        Promise.all([sPanico(empresa, user)]).then(([mail]) => {
          resolve(mail);
        });
      });
    });
...
function sPanico(empresa, user) {  
  return new Promise((resolve, reject) => {
    ...
    let mailOptions = {
      to: user.refemail,
      subject: `Última ubicación`,
      html: `<body><h2>Hola ${user.refnombre} ,</h2>            
